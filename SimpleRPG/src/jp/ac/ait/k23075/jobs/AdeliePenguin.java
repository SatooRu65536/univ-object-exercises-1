...
public class AdeliePenguin extends Hero implements IHeroJob {

    public AdeliePenguin(String name, int hp, int atk, int def, int agi) {
    ...
    /**
     * 5%の確率で敵を誘惑し体力を半減させるが、失敗すると攻撃できない
     * 敵を倒し切ることはできない
     * 
    ...
    public AttackResult specialAttack(CharacterBase target) {
        ...
        int damage = target.getHp() / 2;

        // 攻撃成功時 5% の確率でクリティカル
        if (r.nextDouble() < 0.05) {
            System.out.println(target.getName() + " はキュートなアイリングに誘惑された！");
        } else {
            System.out.println(target.getName() + " は誘惑されなかった！");
            damage = 0;
        }
